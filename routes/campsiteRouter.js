...
campsiteRouter
  ...
  .post(authenticate.verifyUser, (req, res) => {
    res.statusCode = 403;
    res.end(
      `POST operation not supported on /campsites/${req.params.campsite}`
    );
  })
      ...
      .then((campsite) => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
          if (
            ...
            err = new Error(
              `You can't update comment ${req.params.commentId} since you are not the author`
            );
            err.status = 403;
            return next(err);
          }
      ...
      .then((campsite) => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
          if (
            ...
            err = new Error(
              `You can't delete comment ${req.params.commentId} since you are not the author`
            );
            err.status = 403;
            return next(err);
          }
